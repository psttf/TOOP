...
/*  type ParameterType = Call :+: InputValue :+: String :+: CNil

  implicit def pp(x: Object): ParameterType = x match {
    case a: Call => Coproduct[ParameterType](a)
    case a: InputValue => Coproduct[ParameterType](a)
  }*/
...
/*  type Argument = Lambda :+: InputValue :+: Expression :+: CNil
  ...
  final case class Function(param1: ParameterType, operator: Operation, param2: ParameterType) extends ExpressionBody

  final case class Type(args: Seq[Either[Type, String]])

  sealed trait InputValue extends Value with Argument with ParameterType
  final case class IntValue(int: Int) extends InputValue
  final case class RealValue(real: Double) extends InputValue
  final case class StringValue(string: String) extends InputValue with ExpressionBody
  ...
  final case class Call(contextName: Option[String], propertyName: String, arguments: Option[Seq[Argument]]) extends CutExpression with ParameterType
  ...
  val parameter: P[ParameterType] = P(call | inputValue)
  ...
  val value: P[Value] = P(body | inputValue)
  ...
  val exprBody: P[ExpressionBody] = P(fieldUpdate | methodUpdate | function | call | stringValue)
  ...
  try {
    ...
    println(method.parse("retrieve: Obj = @s => s"))
